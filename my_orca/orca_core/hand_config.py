...
from .constants import CONTROL_MODES, DEFAULT_MODEL_NAME, JOINT_IDS, JOINT_LIMITS, JOINT_TO_MOTOR_MAP, MOTOR_IDS
...
class BaseHandConfig:
    ...
    joint_limits_dict: Dict[str, List[float]] = field(default_factory=dict)
    ...
    def from_config_path(
        ...
        if JOINT_LIMITS in config:
            kwargs["joint_limits_dict"] = dict(config[JOINT_LIMITS])
    ...
    def validate(self) -> None:
        ...
        for joint in self.joint_ids:
            if joint not in self.joint_limits_dict:
                raise HandConfigValidationError(f"Limits for joint {joint} is not defined.")

        for joint, limits in self.joint_limits_dict.items():
            if joint not in self.joint_ids:
                raise HandConfigValidationError(f"Joint {joint} in limits is not defined.")
            if len(limits) != 2 or limits[1] - limits[0] <= 0:
                raise HandConfigValidationError(f"Limits {limits} for joint {joint} is not valid.")
    ...
    def clamp_joint_positions(
        ...
        """Clamp joint positions (degrees) to the configured limits."""
    ...
    def get_joint_deg_limits(self, joint_name: str) -> tuple[float, float]:
        """Return (deg_min, deg_max) for a joint, accounting for motor inversion.

        Normal:  raw = (deg/360)*4095       → deg = raw/4095*360
        Inverted: raw = 4095-(deg/360)*4095 → deg = (4095-raw)/4095*360
        """
        min_raw, max_raw = self.joint_limits_dict[joint_name]
        inverted = getattr(self, 'joint_inversion_dict', {}).get(joint_name, False)
        if inverted:
            deg_min = (4095.0 - max_raw) / 4095.0 * 360.0
            deg_max = (4095.0 - min_raw) / 4095.0 * 360.0
        else:
            deg_min = min_raw / 4095.0 * 360.0
            deg_max = max_raw / 4095.0 * 360.0
        return deg_min, deg_max

    def _clamp_joint_value(self, joint_name: str, joint_deg: float) -> float:
        """Clamp joint angle (degrees) to the configured limits."""
        min_deg, max_deg = self.get_joint_deg_limits(joint_name)
        return max(min_deg, min(max_deg, float(joint_deg)))
