...
# Position limits: 15-bit signed range (-32768 to 32767, ~±8 turns)
# Feetech servos support multi-turn positioning through extended position values
POS_MIN = -32768
POS_MAX = 32767
...
class FeetechClient(MotorClient):
    ...
    def __init__(
        ...
        self._multi_turn_configured: set[int] = set()  # Motors with angle limits disabled
    ...
    def connect(self) -> None:
        ...
        # Use TxOnly — no need to wait for status on mode setup
        for motor_id in self.motor_ids:
            self.packet_handler.write1ByteTxOnly(motor_id, SMS_STS_MODE, 0)
    ...
    def set_torque_enabled(
        ...
        Uses TxOnly (no status packet wait) for reliability with STS3215 servos
        which may not respond to TxRx in time. Falls back to TxRx on failure.
        """
        ...
        while remaining_ids:
            ...
            for motor_id in remaining_ids:
                # Prefer TxOnly — faster and more reliable for STS3215
                result = self.packet_handler.write1ByteTxOnly(
                    motor_id, SMS_STS_TORQUE_ENABLE, value
                )
                if result != COMM_SUCCESS:
                    # Fall back to TxRx for error detection
    ...
    def set_operating_mode(self, motor_ids: Sequence[int], mode: int) -> None:
        ...
        Feetech STS/SMS servos have two hardware modes:
        - Mode 0: Servo mode (position control, inherently multi-turn via 15-bit
          position encoding: -32768..32767, ~±8 turns)
        - Mode 1: Wheel mode (velocity control)

        All Dynamixel position-control modes map to servo mode. Multi-turn (mode 4)
        needs no separate mode — the extended position range handles it natively.
        """
        self._check_connected()

        if mode == 0:
            logging.info("Feetech: current control (mode 0) mapped to servo mode")
        if mode == 5:
            logging.info("Feetech: current-based position mode uses torque limiting")
        ...
        for motor_id in motor_ids:
            ...
            # Use TxOnly for mode writes (more reliable than TxRx for STS3215)
            result = self.packet_handler.write1ByteTxOnly(
                motor_id, SMS_STS_MODE, feetech_mode
            )
            if result != COMM_SUCCESS:
                logging.error(
                    'Failed to set operating mode for motor %d: result=%d',
                    motor_id, result
                )

        # Multi-turn (mode 4) requires clearing EEPROM angle limits.
        # Must happen while torque is still off.
        if mode == 4:
            self._configure_multi_turn(motor_ids)

        # Re-enable torque
        self.set_torque_enabled(motor_ids, True)

    def read_position_single(self, motor_id: int) -> float | None:
        """Read position of a single motor. Returns radians or None on failure."""
        self._check_connected()
        ...
        if result == COMM_SUCCESS and error == 0:
            ...
            return float(pos_normalized * self.pos_scale)
    ...
    def read_current_single(self, motor_id: int) -> float | None:
        """Read current of a single motor. Returns mA or None on failure."""
        self._check_connected()
        ...
        if result == COMM_SUCCESS and error == 0:
            ...
            return float(cur_signed * self.cur_scale)
    ...
    def read_pos_vel_cur(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        Tries sync read first (single bus transaction), falls back to
        individual TxRx reads. Failed reads return NaN.
        """
        self._check_connected()

        # Try sync read first — much faster and more reliable for STS3215
        try:
            pos, vel, cur = self.read_pos_vel_cur_sync()
            ok_count = np.sum(~np.isnan(pos))
            if ok_count >= len(self.motor_ids) * 0.5:  # >50% success = usable
                return pos, vel, cur
        except Exception:
            pass

        # Fallback: individual reads
        positions = np.full(len(self.motor_ids), np.nan, dtype=np.float32)
        velocities = np.full(len(self.motor_ids), np.nan, dtype=np.float32)
        currents = np.full(len(self.motor_ids), np.nan, dtype=np.float32)
        ...
        for i, motor_id in enumerate(self.motor_ids):
            try:
                ...
                if result == COMM_SUCCESS and error == 0:
                    ...
                    self._force_clear_busy()
            except Exception:
                pass  # Corrupted response — leave as NaN
    ...
    def write_desired_pos(
        ...
        Uses sync write for multiple motors (single bus transaction).
        Falls back to individual writes for single motors.
        ...
        # Use sync write for multi-motor commands — one bus transaction instead of N
        if len(motor_ids) > 1:
            self.write_positions_sync(motor_ids, positions, speed=speed, torque=torque)
            return

        # Single motor: individual write is fine
    ...
    def _configure_multi_turn(self, motor_ids: Sequence[int]) -> None:
        """Clear EEPROM angle limits for multi-turn operation.

        Feetech servos restrict position by MIN_ANGLE_LIMIT (addr 9-10) and
        MAX_ANGLE_LIMIT (addr 11-12). Factory default 0-4095 limits rotation
        to a single turn. Writing both to 0 unlocks the full 15-bit signed
        position range (-32768..32767, ~±8 turns).

        Caller must disable torque before calling. Only writes to EEPROM
        once per motor (tracked by ``_multi_turn_configured``).
        """
        new_motors = [m for m in motor_ids if m not in self._multi_turn_configured]
        if not new_motors:
            return
        ...
        for motor_id in new_motors:
            ...
            # Use TxOnly — EEPROM writes are slow and TxRx may time out
            result = self.packet_handler.writeTxOnly(
                motor_id, SMS_STS_MIN_ANGLE_LIMIT_L, 4, [0, 0, 0, 0]
            )
            if result != COMM_SUCCESS:
                logging.error(
                    'Failed to clear angle limits for motor %d: result=%d',
                    motor_id, result
                )

            self.packet_handler.LockEprom(motor_id)
            self._multi_turn_configured.add(motor_id)
            logging.info(
                'Motor %d: angle limits cleared for multi-turn operation', motor_id
            )
    ...
    def _force_clear_busy(self) -> None:
        """Force-clear the is_using flag and flush serial buffers.

        COMM_PORT_BUSY errors chain-reaction: one failed read leaves is_using=True,
        then ALL subsequent operations fail with BUSY. This breaks the deadlock.
        """
        if self.packet_handler is not None:
            try:
                self.packet_handler.portHandler.is_using = False
                self.packet_handler.portHandler.clearPort()
            except Exception:
                pass
    ...
    def _clamp_position(self, pos_raw: int) -> int:
        """Clamp position to valid 15-bit signed range."""
        return max(POS_MIN, min(POS_MAX, pos_raw))

    @property
    def requires_offset_calibration(self) -> bool:
        return False  # Feetech reOfsCal shifts readings unpredictably — use raw positions
    ...
    def write_positions_sync(
        ...
        if result != COMM_SUCCESS:
            logging.warning('Sync write failed (result=%d), falling back to individual writes', result)
            # Fall back to individual writes
            for motor_id, pos_rad in zip(motor_ids, positions):
                pos_raw = int(pos_rad / self.pos_scale)
                pos_raw = self._clamp_position(pos_raw)
                w_result, w_error = self.packet_handler.WritePosEx(
                    motor_id, pos_raw, speed, acc, torque,
                )
                if w_result != COMM_SUCCESS or w_error != 0:
                    logging.error(
                        'Failed to write position to motor %d: result=%d, error=%d',
                        motor_id, w_result, w_error,
                    )

        # Clear params for next use
        self.packet_handler.groupSyncWrite.clearParam()

    def write_positions_fast(
        self,
        motor_ids: Sequence[int],
        positions: np.ndarray,
    ) -> None:
        """Write positions with maximum speed for emergency/zeroing operations.

        Uses speed=0 (max), acc=0 (max), torque=500.
        This minimizes the time under load and reduces overcurrent risk
        during critical operations like returning to neutral.
        ...
        self.write_positions_sync(
            motor_ids, positions,
            speed=0,      # 0 = maximum speed
            acc=0,        # 0 = maximum acceleration
            torque=500,
        )

    def sync_read_currents(
        self,
        motor_ids: Sequence[int],
    ) -> dict[int, float | None]:
        """Sync-read present current (mA) for the given motor IDs.

        Uses address PRESENT_CURRENT_L (69), reads 2 bytes.
        Returns dict[motor_id] = mA or None on failure.
        """
        self._check_connected()
        ...
        for mid in motor_ids:
            available, error = sync_read.isAvailable(
                mid, SMS_STS_PRESENT_CURRENT_L, 2
            )
            if available:
                cur_raw = sync_read.getData(mid, SMS_STS_PRESENT_CURRENT_L, 2)
                cur_signed = self.packet_handler.scs_tohost(cur_raw, 15)
                out[int(mid)] = abs(float(cur_signed * self.cur_scale))
            else:
                out[int(mid)] = None
    ...
    def read_pos_vel_cur_sync(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if result != COMM_SUCCESS:
            ...
            # Don't recurse — return NaN array, caller will handle
            return positions, velocities, currents
