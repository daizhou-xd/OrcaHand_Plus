...
def update_yaml(file_path, key, value):
    ...
    try:
        with open(file_path, 'r+', encoding='utf-8') as file:
            ...
            file.seek(0)
            yaml.dump(new_data, file, default_flow_style=False, sort_keys=False, allow_unicode=True)
            file.truncate()
    except FileNotFoundError:
        with open(file_path, 'w', encoding='utf-8') as file:
            yaml.dump({key: value}, file, default_flow_style=False, sort_keys=False, allow_unicode=True)
...
def get_and_choose_port() -> str:
    ...
    Uses ``curses`` on Unix, falls back to a simple numbered list on Windows.

    Returns:
        Device string of the selected port (e.g. ``"COM13"``), or
    ...
    # Try curses first (Unix), fall back to simple text menu (Windows)
    try:
        import curses
    except ImportError:
        return _choose_port_simple(ports)
    return _choose_port_curses(ports, curses)


def _choose_port_simple(ports) -> str | None:
    """Simple numbered text menu for port selection (Windows compatible)."""
    print("\nAvailable serial ports:")
    for i, p in enumerate(ports):
        print(f"  {i+1}. {p.device} — {p.description or 'unknown'}")
    print(f"  0. Cancel")
    print()
    while True:
        try:
            choice = input(f"Select port (1-{len(ports)}, 0=cancel): ").strip()
            if choice == "0":
                return None
            idx = int(choice) - 1
            if 0 <= idx < len(ports):
                return ports[idx].device
            print(f"Invalid choice. Enter 1-{len(ports)} or 0.")
        except (ValueError, EOFError, KeyboardInterrupt):
            return None


def _choose_port_curses(ports, curses) -> str | None:
