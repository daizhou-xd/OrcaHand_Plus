...
> 正确入口：请从 `D:\Github_Project\OrcaHand_Plus\orca-feetech-STS3215` 运行本项目。`my_orca` 不是当前可用上位机，不要再作为改造或运行入口。
...
- 一个状态可视化入口：GUI 内置简化手部状态视图，不影响串口控制链路。
...
如果你当前在总仓库根目录 `D:\Github_Project\OrcaHand_Plus`，先进入真正的上位机目录：

```powershell
cd D:\Github_Project\OrcaHand_Plus\orca-feetech-STS3215
```

下面所有命令都在 `orca-feetech-STS3215` 目录运行（也就是有 `py_action/` 的那个目录）。
...
4. 右侧“手部状态”会根据舵机位置回读显示简化姿态，不影响串口控制、动作执行或急停。
5. 遇到异常随时点“急停”（会回零并恢复手动控制）。
...
### Q5：手部状态视图是否会影响控制？
不会。右侧“手部状态”只是读取 GUI 已回读到的舵机位置并绘制简化姿态，不参与串口写入，也不会影响急停、过流保护、动作执行、API 或相机入口。
