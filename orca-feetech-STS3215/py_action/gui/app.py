...
cfg_panel = tk.Frame(config_frame, bg=PANEL)
cfg_panel.pack(padx=15, pady=8, fill="x")
...
port_row = tk.Frame(cfg_panel, bg=PANEL)
port_row.pack(pady=5, fill="x")

port_inner = tk.Frame(port_row, bg=PANEL)
port_inner.pack(anchor="center")

tk.Label(port_inner, text="串口号：", bg=PANEL).pack(side="left")
...
if available_ports:
    port_menu = tk.OptionMenu(port_inner, port_var, available_ports[0], *available_ports[1:])
else:
    port_menu = tk.OptionMenu(port_inner, port_var, "")
...
def set_debug_mode(enabled: bool):
    ...
    if enabled:
        ...
        # 自动启动 3D 手部模型（若未启动）
        if _viewer3d is None or not _viewer3d.is_running():
            _start_3d_viewer()
...
# ======================
tool_panel = tk.Frame(config_frame, bg=PANEL)
tool_panel.pack(padx=15, pady=8, fill="both", expand=False)

tk.Label(tool_panel, text="工具箱（总控台）", font=("微软雅黑", 12, "bold"), bg=PANEL).pack(pady=6)
...
tk.Label(tool_panel, text=warn, bg=PANEL, fg="#666666", justify="left", font=("微软雅黑", 9)).pack(pady=2)

row_cfg = tk.Frame(tool_panel, bg=PANEL)
row_cfg.pack(fill="x", padx=6, pady=6)

tk.Label(row_cfg, text="API base:", bg=PANEL).grid(row=0, column=0, sticky="w")
base_var = tk.StringVar(value="http://127.0.0.1:8001")
tk.Entry(row_cfg, textvariable=base_var, width=36).grid(row=0, column=1, sticky="we", padx=6)

tk.Label(row_cfg, text="Port:", bg=PANEL).grid(row=0, column=2, sticky="e")
port_api_var = tk.StringVar(value="8001")
tk.Entry(row_cfg, textvariable=port_api_var, width=8).grid(row=0, column=3, sticky="e")

tk.Label(row_cfg, text="Config:", bg=PANEL).grid(row=1, column=0, sticky="w", pady=4)
config_path_var = tk.StringVar(value="py_action/config_orca.yaml")
tk.Entry(row_cfg, textvariable=config_path_var, width=36).grid(row=1, column=1, sticky="we", padx=6, pady=4)
...
tk.Label(row_cfg, text="Camera:", bg=PANEL).grid(row=2, column=0, sticky="w", pady=4)
camera_index_var = tk.StringVar(value="0")

cam_row = tk.Frame(row_cfg, bg=PANEL)
cam_row.grid(row=2, column=1, sticky="w", padx=6, pady=4)
...
def _refresh_tool_status():
    ...
    parts.append(f"3D:{'RUN' if _is_proc_running(cam3d_proc) else 'STOP'}")
...
row_btn.pack(fill="x", padx=6, pady=4)
...
tool_status_var = tk.StringVar(value="API:STOP | 3D:STOP")
tk.Label(tool_panel, textvariable=tool_status_var, bg=PANEL, fg="#333333", font=("微软雅黑", 10, "bold")).pack(pady=4)

log_frame = tk.Frame(tool_panel, bg=PANEL)
log_frame.pack(fill="both", expand=True, padx=6, pady=6)

log_text = tk.Text(log_frame, height=12, wrap="word", bg="#111111", fg="#DDDDDD")
log_text.configure(state="disabled")
...
def _start_3d_viewer():
    ...
    if urdf_viewer is None:
        messagebox.showwarning("缺少依赖", "3D 模型需要 yourdfpy / pyglet，请先安装：pip install yourdfpy pyglet")
        return
...
def _refresh_overcurrent_label_colors():
    ...
    for sid, lbl in servo_id_labels.items():
        try:
            lbl.config(fg=("#D32F2F" if int(sid) in tripped else "#333333"))
        except Exception:
            pass
...
# 3D 模型（URDF）—— 独立窗口，同步关节弯曲
model3d_frame = tk.Frame(right_col, bg=PANEL)
model3d_frame.pack(fill="x", expand=False, pady=(6, 3))
...
_model3d_btn = tk.Button(
    ...
    relief="flat",
    height=2,
    command=_start_3d_viewer,
)
_model3d_btn.pack(fill="x", padx=12, pady=(2, 4))
